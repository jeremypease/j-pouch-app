...
- One-time after cloning: `git config core.hooksPath .githooks` — activates the pre-push hook that runs the `senior-apple-reviewer` subagent (`.claude/agents/senior-apple-reviewer.md`) on outgoing commits and blocks pushes with build-breaking findings. Requires the `claude` CLI on PATH; degrades to a no-op warning if it's missing. Bypass a single push with `git push --no-verify`.
